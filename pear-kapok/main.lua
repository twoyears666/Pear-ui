-- Pear 启动器 · 晨露木棉·暖色 UI 包 —— v0.1.0-beta
--
-- 契约（引擎通用，零特例）：
--   主题：晨露木棉——暖琥珀顶栏 topbar #D97706、内容区暖奶油对角渐变
--         background_start #FFF7E8 → background_end #FBEBD3、白卡 background_card #FFFFFF、
--         深棕文字 cardText #3A2C1E。禁止任何黑色/深色/透明兜底（背景必须在 colors.json）。
...
function describe()
  return { name = "晨露木棉", version = "0.1.0-beta" }
end

local C = {
  topbar      = "$color:topbar",              -- 顶栏暖琥珀 #D97706
  pageFrom    = "$color:pageFrom",            -- 渐变左上 #FFF7E8
  pageTo      = "$color:pageTo",              -- 渐变右下 #FBEBD3
  bg          = "$color:background_start",    -- 页面底色起点 #FFF7E8（渐变同 background_gradient）
  card        = "$color:card",                -- 卡片 #FFFFFF = background_card
  cardBorder  = "$color:cardBorder",          -- 卡片描边 #EAD9BD
  dark        = "$color:cardText",            -- 主文字 #3A2C1E
  mid         = "$color:subText",             -- 次要文字 #9A866A
  white       = "$color:white",
  transparent = "$color:transparent",
  accent      = "$color:accent",              -- 强调琥珀橙 #E0702A
  accentBorder = "$color:accentBorder",
  hover       = "$color:hover",               -- hover 暖麦色 #FFF1DE
  ...
  hintBg      = "$color:faintBlue",           -- 提示条暖麦底 #FFF1DE
  faintBlue   = "$color:faintBlue",           -- 暖麦底（胶囊按钮/图标底）
...
function build(ui)
  ...
  return ui.column {
    ...
    children = {
      -- 顶栏：暖琥珀渐变品牌头，页签选中态套白底全圆药丸（不做绝对定位游标）
      ui.row { id = "titlebar", height = "8.5vh",
        background = { from = C.topbar, to = C.brandOrange, angle = 90 },
        crossAlign = "center",
        padding = { left = "2vh", right = "1.5vh" },
        children = {
          ui.text { id = "logo", text = "木棉", width = "10vh", style = { font = "3.2vh", weight = "bold", color = C.white } },
          ui.spacer { weight = 1 },
