-- Pear 启动器 · 竹影薄荷 UI 包 —— v0.1.0-beta
--
-- 契约（引擎通用，零特例）：
--   主题：竹影薄荷——青绿/薄荷渐变品牌头 topbar → accent、内容区浅薄荷绿对角渐变
--         background_start #EEF8F1 → background_end #D9EEE1、白卡 background_card #FFFFFF、
--         深绿文字 cardText #24362A。禁止任何黑色/深色/透明兜底（背景必须在 colors.json）。
...
function describe()
  return { name = "竹影薄荷", version = "0.1.0-beta" }
end

local C = {
  topbar      = "$color:topbar",              -- 品牌头薄荷绿 #2F8F6B
  pageFrom    = "$color:pageFrom",            -- 渐变左上 #EEF8F1
  pageTo      = "$color:pageTo",              -- 渐变右下 #D9EEE1
  bg          = "$color:background_start",    -- 页面底色起点 #EEF8F1（渐变同 background_gradient）
  card        = "$color:card",                -- 卡片 #FFFFFF = background_card
  cardBorder  = "$color:cardBorder",          -- 卡片描边 #CBE2D3
  dark        = "$color:cardText",            -- 主文字 #24362A
  mid         = "$color:subText",             -- 次要文字 #7E9387
  white       = "$color:white",
  transparent = "$color:transparent",
  accent      = "$color:accent",              -- 强调薄荷绿 #2AA36B
  accentBorder = "$color:accentBorder",
  hover       = "$color:hover",               -- hover 浅绿 #E9F6EE
  ...
  hintBg      = "$color:faintBlue",           -- 提示条浅绿底 #E9F6EE
  faintBlue   = "$color:faintBlue",           -- 浅绿底（胶囊按钮/图标底）
...
function build(ui)
  ...
  return ui.column {
    ...
    children = {
      -- 顶栏：青绿/薄荷渐变品牌头，页签选中态套白底全圆药丸（不做绝对定位游标）
      ui.row { id = "titlebar", height = "8.5vh", background = { from = C.topbar, to = C.accent, angle = 30 }, crossAlign = "center",
        padding = { left = "2vh", right = "1.5vh" },
        children = {
          ui.text { id = "logo", text = "薄荷", width = "10vh", style = { font = "3.2vh", weight = "bold", color = C.white } },
          ui.spacer { weight = 1 },
